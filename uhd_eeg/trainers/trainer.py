...
def fit_decoder_CV(args: DictConfig, dataset: EEGDataset | EMGDataset) -> None:
    ...
    for cv, (ind_tr, ind_val) in enumerate(skf.split(inds, dataset.labels_all)):
        ...
        if args.model_name == "EEGNet":
            net = EEGNet(args, T=dataset.window_eegnet)
        elif args.model_name == "EEGNet_with_mask":
            net = EEGNet_with_mask(args, T=dataset.window_eegnet)
        ...
        if args.model_name == "RNN":
            summary(
                net,
                input_size=(args.batch_size, dataset.window_eegnet, args.num_channels),
            )
        elif args.model_name == "CovTanSVM":
            print(clf)
        else:
            summary(net, input_size=(1, 1, args.num_channels, dataset.window_eegnet))
        ...
        if args.model_name == "CovTanSVM":
            ...
            if args.save_results:
                filename = (
                    f"CovTanSVM_{args.config_name}_N{args.n_trial_avg}_cv{cv}.dill"
                )
            ...
            for epoch in range(args.n_epochs):  # loop over the dataset multiple times
                ...
                for i, data in enumerate(train_loader, 0):
                    net.train()
                    inputs, labels = data
                    inputs = inputs.to(dataset.device)
                    labels = labels.to(dataset.device)
                    if args.no_avg:
                        inputs = torch.concat([inp for inp in inputs])
                        labels = labels.repeat_interleave(args.n_trial_avg)
                    if args.model_name == "RNN":
                        # original inputs shape: (batch_size, 1, num_channels, window_eegnet)
                        # RNN inputs shape: (batch_size, window_eegnet, num_channels)
    ...
    if args.model_name == "CovTanSVM":
        if args.save_results:
            ...
            if args.use_hydra_savedir:
                np.save(
                    f"acc_tr_{args.config_name}_avg{args.n_trial_avg}.npy", acc_tr_all
                )
                np.save(
                    f"acc_val_{args.config_name}_avg{args.n_trial_avg}.npy", acc_val_all
                )
            else:
                np.save(
                    f"{dir_save}/acc_tr_{args.config_name}_avg{args.n_trial_avg}.npy",
                    acc_tr_all,
                )
                np.save(
                    f"{dir_save}/acc_val_{args.config_name}_avg{args.n_trial_avg}.npy",
                    acc_val_all,
                )
        ...
        if args.use_hydra_savedir:
            np.save(
                f"loss_tr_{args.config_name}_avg{args.n_trial_avg}.npy", loss_tr_log
            )
            np.save(
                f"loss_val_{args.config_name}_avg{args.n_trial_avg}.npy", loss_val_log
            )
            np.save(f"acc_tr_{args.config_name}_avg{args.n_trial_avg}.npy", acc_tr)
            np.save(f"acc_val_{args.config_name}_avg{args.n_trial_avg}.npy", acc_val)
        else:
            np.save(
                f"{dir_save}/loss_tr_{args.config_name}_avg{args.n_trial_avg}.npy",
                loss_tr_log,
            )
            np.save(
                f"{dir_save}/loss_val_{args.config_name}_avg{args.n_trial_avg}.npy",
                loss_val_log,
            )
            np.save(
                f"{dir_save}/acc_tr_{args.config_name}_avg{args.n_trial_avg}.npy",
...
@hydra.main(
    ...
    config_name="config_color_multirun.yaml",
)
