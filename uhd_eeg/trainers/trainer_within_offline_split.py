    ...
    for cv, (ind_tr, ind_val) in enumerate(skf.split(inds, labels_train)):
        ...
        if args.model_name != "CovTanSVM":
            ...
            if args.optimizer == "AdamW":
                ...
                optimizer = optim.Adam(
                    net.parameters(), lr=args.learning_rate, eps=args.eps
                )
        ...
        if args.model_name == "CovTanSVM":
            ...
            if args.save_results:
                filename = f"CovTanSVM_cv{cv}.dill"
            ...
            if args.save_results:
                ...
                if args.use_hydra_savedir:
                    torch.save(net.state_dict(), f"weight_cv{cv}.pth")
                else:
                    torch.save(net.state_dict(), f"{dir_save}/weight_cv{cv}.pth")
    if args.model_name == "CovTanSVM":
        if args.save_results:
            acc_tr_all = np.array(acc_tr_all)
            acc_val_all = np.array(acc_val_all)
            if args.use_hydra_savedir:
                np.save(f"acc_tr.npy", acc_tr_all)
                np.save(f"acc_val.npy", acc_val_all)
            else:
                np.save(f"{dir_save}/acc_tr.npy", acc_tr_all)
                np.save(f"{dir_save}/acc_val.npy", acc_val_all)
        print("Finished Training", flush=True)
        print(f"acc_tr_all: {acc_tr_all}", flush=True)
        print(f"acc_val_all: {acc_val_all}", flush=True)
    else:
        if args.use_hydra_savedir:
            np.save(f"loss_tr.npy", loss_tr_log)
            np.save(f"loss_val.npy", loss_val_log)
            np.save(f"acc_tr.npy", acc_tr)
            np.save(f"acc_val.npy", acc_val)
        else:
            np.save(f"{dir_save}/loss_tr.npy", loss_tr_log)
            np.save(f"{dir_save}/loss_val.npy", loss_val_log)
            np.save(f"{dir_save}/acc_tr.npy", acc_tr)
            np.save(f"{dir_save}/acc_val.npy", acc_val)
        print("dataset size: ", len(dataset))
        print(
...
def run(args: DictConfig) -> None:
    ...
    indices_train = np.arange(
        int((1 - args.test_ratio) * np.round(len(dataset))), dtype=int
    )
    indices_test = np.arange(len(dataset))[len(indices_train) :]
    dataset_train = Subset(dataset, indices_train)
    dataset_test = Subset(dataset, indices_test)
    labels_all = dataset.labels_all
    labels_train = np.array([labels_all[ind] for ind in indices_train])
    labels_test = np.array([labels_all[ind] for ind in indices_test])
    ...
    within_offline_split_test_save(
        args=args,
        dataset_test=dataset_test,
        n_worekers=args.n_worekers,
        device=device,
    )

    # idx = np.where(DATA_LIST["dataname"] == args.parallel_sets)[0]
    # assert len(idx) == 1
    # idx = idx[0]
    # subject = DATA_LIST["subject"][idx]
    # task = DATA_LIST["task"][idx]
    # print(f"subject: {subject}, task: {task}")

    # dir_src = Path(
    #     args.gmail.npy_dir.replace("eeg_margin_before_preproc", "eeg_after_preproc")
    # )
    # dir_dst = Path(
    #     args.gmail.npy_dir.replace(
    #         "eeg_margin_before_preproc", "eeg_after_preproc_test"
    #     )
    # )
    # dir_dst.mkdir(exist_ok=True, parents=True)
    # for idx in indices_test:
    #     target_files = list(dir_src.glob(f"{idx}.pt"))
    #     assert len(target_files) == 1
    #     target_file = target_files[0]
    #     filename = str(target_file).split("/")[-1]
    #     print(target_file)
    #     print(dir_dst / filename)
    #     # shutil.copy(target_file, dir_dst / filename)
    # # save labels_test .csv
    # # np.savetxt(dir_dst / f"word_list_{subject}_{task}.csv", labels_test, delimiter=",")
    # # delete word_list.csv
    # if (dir_dst / "word_list.csv").exists():
    #     os.remove(dir_dst / "word_list.csv")
